            ...
            inputs=[
                ...
                io.Int.Input(
                    ...
                    tooltip="Only read when `window_ref_video` is on. Wire the sampler's "
                            "own `overlap_frames`."),
                io.String.Input(
                    "embeddings", optional=True, force_input=True,
                    tooltip="H3 text embeddings to PREPEND to each chunk's prompt. A "
                            "socket, not a text box: wire MMH3 Embedding Select here, "
                            "one node per embedding, chained through `previous`. A "
                            "model file gets PICKED rather than typed, because a name "
                            "that does not resolve is dropped by core with nothing but "
                            "a log line.\n\n"
                            "Each Select node carries its own `chunks` field, so the "
                            "chain schedules as well as stacks -- `all`, `3`, `4-6`, "
                            "1-based. Scheduling works because every chunk already has "
                            "its own prompt, so it is only a question of which prompts "
                            "get the token. Stacked embeddings are additive in both "
                            "cost and effect.\n\n"
                            "They are NOT free -- 50 to 142 token slots each, attended "
                            "at every sampling step of every chunk they are on. The "
                            "report prints the per-chunk total.\n\n"
